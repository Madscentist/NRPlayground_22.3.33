...
public class CarAudioV3 : MonoBehaviour
{
    public CarControllerV3 carController;

    [Header("Engine audio")] [Tooltip("Looping engine sound clip.")]
    public AudioClip engineSoundClip;

    [Tooltip(
        "The pitch of the engine relative to the speed delta of the car. The speed delta is calculated by dividing the current speed by the max speed.")]
    public AnimationCurve enginePitchOverSpeed = new AnimationCurve(new Keyframe(0, 0.25f), new Keyframe(1, 1));

    [Header("Brakes and drifting audio")] [Tooltip("Looping braking sound clip.")]
    public AudioClip brakeSoundClip;

    [Tooltip(
        "The volume of the brake sound relative to the speed delta of the car. The speed delta is calculated by dividing the current speed by the max speed.")]
    public AnimationCurve driftOverSpeed = new AnimationCurve(new Keyframe(0.25f, 0), new Keyframe(0.75f, 1));

    [Header("Bumping audio")] [Tooltip("Collision (bumping) sound clip.")]
    ...
    [Tooltip(
        "The maximum relative velocity at which to play the collision sound. Velocities over this number will still play the sound but clamp the volume to 1.")]
    public float bumpMaxForce = 10;

    [Header("Grinding audio")] [Tooltip("Looping wall grinding sound clip.")]
    ...
    [Tooltip(
        "The maximum relative velocity at which to play the grinding sound. Velocities over this number will still play the sound but clamp the volume to 1.")]
    ...
    [Header("Landing audio")] [Tooltip("Car landing sound clip.")]
    ...
    [Tooltip(
        "The maximum relative velocity at which to play the landing sound. Velocities over this number will still play the sound but clamp the volume to 1.")]
    ...
    void Update()
    {
        ...
        float mainDriftDelta = (carController._isDrifting ? 1f : 0f) * speedDelta;

        sourceBrake.volume =
            Mathf.Clamp01(getDriftVolume(brakeDelta) + getDriftVolume(driftDelta) + getDriftVolume(mainDriftDelta));
        ...
        {
            grindVolume =
                Mathf.Clamp01((carController.getSideHitForce() - grindMinForce) / (grindMaxForce - grindMinForce));
        }
        ...
        {
            float landingVolume = Mathf.Clamp01((carController.getGroundHitForce() - landingMinForce) /
                                                (landingMaxForce - landingMinForce));
