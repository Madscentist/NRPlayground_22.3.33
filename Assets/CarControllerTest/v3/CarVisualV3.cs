...
{
    public class CarVisualV3 : MonoBehaviour
    {
        ...
        [Header("Visuals")] [Tooltip("Object on which to apply the controller's position and rotation.")]
        ...
        [Header("Particles")] [Tooltip("Minimum velocity when scraping against a wall to play the particle system.")]
        ...
        public List<TrailRenderer> TireTrailRenderers;
        ...
        void FixedUpdate()
        {
            ...
            {
                Quaternion groundRotationPlane = Quaternion.Euler(groundRotationSmooth.eulerAngles.x,
                    carController.getGroundRotation().eulerAngles.y, groundRotationSmooth.eulerAngles.z);
                float rotationLerpValue = carController.isGrounded() ? rotationSmoothingOnGround : rotationSmoothingInAir;
                groundRotationSmooth = Quaternion.Slerp(groundRotationPlane, carController.getGroundRotation(),
                    rotationLerpValue <= 0 ? 1 : deltaTime * rotationLerpValue);
            ...
            wheelSpin += carController.getForwardVelocity() * deltaTime * wheelsSpinForce *
                         (adjustToScale ? averageScale : 1);
            wheelRotation = Mathf.Lerp(wheelRotation, carController.getSteering() * wheelsMaxTurnAngle,
                wheelsTurnSmoothing <= 0 ? 1 : Mathf.Clamp01(wheelsTurnSmoothing * deltaTime));
            ...
            {
                if (Mathf.Abs(carController.getLateralVelocity()) > minDriftingSpeed * (adjustToScale ? averageScale : 1) &&
            ...
            {
                if (carController.isHittingSide() && carController.getGroundVelocity() >
                    minSideFrictionVelocity * (adjustToScale ? averageScale : 1))
                {
            ...
            foreach (var trailRenderer in TireTrailRenderers)
            {
                trailRenderer.emitting = carController._isDrifting && carController.isGrounded();
            }
        }
        ...
        {
            if (particlesSideFriction != null)
                particlesSideFriction.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
            if (particlesSideCollision != null)
                particlesSideCollision.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
